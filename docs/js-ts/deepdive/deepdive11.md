...
숫자 값은 어떤 값도 동일한 8바이트가 필요하지만 문자열의 경우 1개의 문자로 이뤄진 문자열은 2바이트, 10개로 문자로 이뤄진 문자열은 20바이트가 필요하다.
...
그리고 두번째 문이 실행되면 'Hello'를 수정하는 것이 아니라 새**로운 문자열 'world'를 메모리에 생성**하고 `str`은 이것을 가리킨다.
